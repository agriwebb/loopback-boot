...
function findConfigFiles(appRootDir, env, name) {
  ...
  function ifExistsAndCanResolve(fileName, ext) {
    return ((ext in require.extensions) && ifExists(fileName + ext)) || undefined;
  }

  function ifExistsWithAnyExt(fileName) {
    return ifExists(fileName + '.js') || ifExists(fileName + '.json') || ifExistsAndCanResolve(fileName, '.ts');
  }
}
