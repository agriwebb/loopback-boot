...
function isExcludedFilePattern(filename) {
  var tests = [
    /\.d\.ts$/
  ];

  return tests.some(t => t.test(filename));
}
...
function isPreferredExtension(filename, includeExtensions) {
  ...
  return !isExcludedFilePattern(filename) && (ext in includeExtensions) && !(ext in getExcludedExtensions());
}
